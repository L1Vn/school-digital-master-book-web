// API Base URL
const API_URL = process.env.NEXT_LOCAL_API_URL || "http://127.0.0.1:8000/api";
...
export const createAlumni = createAlumnus;
...
export async function getClassrooms(params = {}) {
  const query = new URLSearchParams(params).toString();
  return apiRequest(`/classrooms${query ? `?${query}` : ""}`);
}
...
export async function getGradeSummary(studentId, academicYearId, semester) {
  return apiRequest(`/grade-summaries/${studentId}/${academicYearId}/${semester}`);
}
...
export async function getClassSummary(studentId, academicYearId, semester) {
  return apiRequest(`/wali/grade-summaries/${studentId}/${academicYearId}/${semester}`);
}
...
export async function publicGetStudents(params = {}) {
  // Normalize params and enforce default limit 10
  const queryParams = typeof params === 'string' ? { search: params } : { ...params };
  if (!queryParams.limit) queryParams.limit = 10;

  const query = new URLSearchParams(queryParams).toString();
  return fetch(`${API_URL}/public/students${query ? `?${query}` : ""}`).then((res) => res.json());
}
...
export async function publicGetAlumni(params = {}) {
  // Normalize params and enforce default limit 10
  const queryParams = typeof params === 'string' ? { search: params } : { ...params };
  if (!queryParams.limit) queryParams.limit = 10;

  const query = new URLSearchParams(queryParams).toString();
  return fetch(`${API_URL}/public/alumni${query ? `?${query}` : ""}`).then((res) => res.json());
}
